...
function OpenBossMenu2(society, close, options)
	...
	ESX.TriggerServerCallback('esx_society:isBoss', function(isBoss)
		if isBoss then
			...
			if options.salary then
				elements[#elements+1] = {icon = "fas fa-wallet", title = TranslateCap('salary_management2'), value = "manage_salary2"}
			end
			if options.grades then
				elements[#elements+1] = {icon = "fas fa-scroll", title = TranslateCap('grade_management2'), value = "manage_grades2"}
			end
			...
			ESX.OpenContext("right", elements, function(menu,element)
				...
				elseif element.value == "manage_salary2" then
					OpenManageSalaryMenu2(society, options)
				elseif element.value == "manage_grades2" then
...
function OpenManageSalaryMenu2(society, options)
	ESX.TriggerServerCallback('esx_society:getJob2', function(job)
		...
		local elements = {
			{unselectable = true, icon = "fas fa-wallet", title = TranslateCap('salary_management2')}
		}
...
function OpenManageGradesMenu2(society, options)
	ESX.TriggerServerCallback('esx_society:getJob2', function(job)
		...
		local elements = {
			{unselectable = true, icon = "fas fa-wallet", title = TranslateCap('grade_management2')}
		}
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			if element.value == "confirm" then
				if menu.eles[2].inputValue then
					...
					ESX.TriggerServerCallback('esx_society:setJob2Label', function()
					OpenManageGradesMenu2(society, options)
				end, society, menu.eles[1].value, label)
				else
					ESX.ShowNotification(TranslateCap('invalid_value_nochanges'))
