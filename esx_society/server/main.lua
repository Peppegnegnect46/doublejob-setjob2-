...
AddEventHandler('esx_society:getSocieties2', function(cb)
	cb(RegisteredSocieties)
end)
...
ESX.RegisterServerCallback('esx_society:getEmployees2', function(source, cb, society)
	...
	function(result)
		for k, row in pairs(result) do
			...
			for k, v in pairs(employees2) do
				if v.identifier == identifier then
					alreadyInTable = true
				end
			end
...
ESX.RegisterServerCallback('esx_society:setJob2', function(source, cb, identifier, job2, grade, actionType)
	local xPlayer = ESX.GetPlayerFromId(source)
	local isBoss = Config.BossGrades[xPlayer.job2.grade_name]
	local xTarget = ESX.GetPlayerFromIdentifier(identifier)
	...
	if not xTarget then
		MySQL.update('UPDATE users SET job = ?, job_grade = ? WHERE identifier = ?', {job2, grade, identifier},
		function()
			cb()
		end)
		return
...
ESX.RegisterServerCallback('esx_society:setJob2Salary', function(source, cb, job2, grade, salary)
	...
	if xPlayer.job2.name == job and Config.BossGrades[xPlayer.job2.grade_name] then
		if salary <= Config.MaxSalary then
...
ESX.RegisterServerCallback('esx_society:setJob2Label', function(source, cb, job2, grade, label)
	...
	if xPlayer.job2.name == job2 and Config.BossGrades[xPlayer.job2.grade_name] then
		...
		print(('[^3WARNING^7] Player ^5%s^7 attempted to setJobLabel for ^5%s^7!'):format(source, job2))
		cb()
	end
end)
...
ESX.RegisterServerCallback('esx_society:isBoss', function(source, cb, job2)
...
function isPlayerBoss(playerId, job2)
	local xPlayer = ESX.GetPlayerFromId(playerId)
