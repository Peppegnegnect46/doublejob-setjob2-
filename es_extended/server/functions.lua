...
function ESX.CreateJob2(name, label, grades)
	if not name then
		return print('[^3WARNING^7] missing argument `name(string)` while creating a job2')
	end

	if not label then
		return print('[^3WARNING^7] missing argument `label(string)` while creating a job2')
	end

	if not grades or not next(grades) then
		return print('[^3WARNING^7] missing argument `grades(table)` while creating a job2!')
	end
	...
	for _, v in pairs(grades) do
		job2.grades[tostring(v.grade)] = { job2_name = name, grade = v.grade, name = v.name, label = v.label, salary = v.salary, skin_male = {}, skin_female = {} }
		parameters[#parameters + 1] = { name, v.grade, v.name, v.label, v.salary }
	end

	MySQL.insert('INSERT IGNORE INTO jobs2 (name, label) VALUES (?, ?)', { name, label })
	MySQL.prepare('INSERT INTO job2_grades (job2_name, grade, name, label, salary) VALUES (?, ?, ?, ?, ?)', parameters)
...
function ESX.RefreshJobs2()
	...
	local jobGrades = MySQL.query.await('SELECT * FROM job2_grades')
	...
	if not Jobs2 then
		-- Fallback data, if no jobs exist
		ESX.Jobs2['nojob2'] = { label = 'NoGang', grades = { ['0'] = { grade = 0, label = 'NoGang', salary = 200, skin_male = {}, skin_female = {} } } }
	else
		ESX.Jobs2 = Jobs2
	end
end
...
function ESX.DoesJob2Exist(job2, grade)
	...
	if job2 and grade then
		if ESX.Jobs2[job2] and ESX.Jobs2[job2].grades[grade] then
			return true
		end
	end
