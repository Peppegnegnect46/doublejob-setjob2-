...
if Config.Multichar then
    AddEventHandler('esx:onPlayerJoined', function(src, char, data)
        local timer = 0
        while (not next(ESX.Jobs) and not next(ESX.Jobs2)) and timer < 5000 do -- Timeout di 5 secondi
            Wait(50)
            timer = timer + 50
        end
...
AddEventHandler("esx:setJob2", function(_, job2, lastJob)
	local lastJobKey = ('%s:count'):format(lastJob.name)
	local job2Key = ('%s:count'):format(job2.name)
	local currentLastJob = ESX.JobsPlayerCount[lastJob.name]

	ESX.JobsPlayerCount[lastJob.name] = ((currentLastJob and currentLastJob > 0) and currentLastJob or 1) - 1
	ESX.JobsPlayerCount[job2.name] = (ESX.JobsPlayerCount[job2.name] or 0) + 1

	GlobalState[lastJobKey] = ESX.JobsPlayerCount[lastJob.name]
	GlobalState[job2Key] = ESX.JobsPlayerCount[job2.name]
end)
